...
use stateless_validator_common::new_payload_request::{
    ExecutionPayloadV3, NewPayloadRequestElectraFulu, Withdrawal, compute_requests_hash,
};

use crate::execution_payload::{
    EncodedTransaction, ExecutionPayload, validate_block_payload_v3, validate_execution_payload_v3,
};

/// Converts a [`NewPayloadRequest`] into an ethrex [`Block`].
pub fn get_block_from_new_payload_request(
    req: NewPayloadRequestElectraFulu,
    hasher: &impl Sha256Hasher,
    crypto: &dyn Crypto,
) -> Result<Block> {
    let parent_beacon_block_root = Some(H256::from(req.parent_beacon_block_root));
    let requests_hash = Some(H256::from(compute_requests_hash(
        &req.execution_requests,
        hasher,
    )));
    let payload: ExecutionPayload = req.execution_payload.into();
    ...
    validate_block_payload_v3(&payload, &block, &req.versioned_hashes)
